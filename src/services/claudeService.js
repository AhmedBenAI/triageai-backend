...
async function callLLM(systemPrompt, userContent, stageName) {
  const start = Date.now();
  logger.info(`LLM call started: ${stageName} [claude]`);
  ...
  const costUsd = calculateCost(inputTokens, outputTokens, "claude");
  const text = response.content.map((b) => b.text || "").join("");

  logger.info(`LLM call completed: ${stageName} [claude]`, {
    latencyMs, inputTokens, outputTokens, costUsd: costUsd.toFixed(6),
  });
...
Analyse the incoming ticket and respond ONLY with a valid JSON object — no markdown, no explanation.
...
- low: general enquiries, feature requests`;
  ...
  const result = await callLLM(systemPrompt, ticketText, "classify");
  ...
  try {
    ...
    classification = {
      ...
      sentiment: "neutral", summary: ticketText.slice(0, 80), intent: "resolve issue",
    };
  }
...
- flag: true ONLY if response is factually wrong, harmful, or completely off-topic`;
  ...
  try {
    ...
    evaluation = {
      relevance: 0.8, completeness: 0.8, tone: 0.85,
      actionability: 0.8, overall: 0.81, flag: false, flag_reason: "",
    };
  }
